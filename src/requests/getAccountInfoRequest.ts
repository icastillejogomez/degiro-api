...
const { GET_ACCOUNT_INFO } = DEGIRO_API_PATHS
...
export function getAccountInfoRequest(accountData: AccountDataType, accountConfig: AccountConfigType): Promise<AccountInfoType> {
  return new Promise((resolve, reject) => {
    ...
    const uri = `${accountConfig.data.tradingUrl}${GET_ACCOUNT_INFO}${accountData.data.intAccount};jsessionid=${accountConfig.data.sessionId}`
